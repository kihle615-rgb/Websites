...
Phone number, address, and closing time are in `index.html`. The number appears
...
The listed closing time is 21:00. Full trading hours were not confirmed, so only
the closing time is shown — add the rest in the "Visit" section when you have it.
...
- **Type** is Amiri for headings — a Naskh-revival face whose Latin was drawn
  alongside Arabic, so it is related to the calligraphy in the logo — with Jost
  for body text.
